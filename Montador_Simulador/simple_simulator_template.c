//MUDANCA NO STATE DECODE:
//adição do switch POW

//MUDANÇA NA FUNÇÃO ULA:
//adição do switch POW com a operação
...
#define POW 38      // "100110"; -- POW Rx Ry 				-- Rx <- -Ry  Format: < inst(6) | Rx(3) | Ry(3) | xxxx >
...
int kbhit(void)
{
    return _kbhit(); // Função nativa do Windows (conio.h)
}
...
loop:
	...
	switch(state)
	{
		...
		case STATE_HALTED:
            ...
            // Imprime as Flags (Overflow, Zero, Negativo, etc)
            printf("Flags (FR): Overflow=%d, DivZero=%d, Neg=%d, Zero=%d\n", 
                    FR[ARITHMETIC_OVERFLOW], FR[DIV_BY_ZERO], FR[NEGATIVE], FR[ZERO]);
            
			printf("\nPressione 'q' para sair ou 'r' para reiniciar...\n");
			
            // Usa o seu _kbhit() adaptado ou getchar()
			key = getchar();
	...
	switch(pega_pedaco(OP, 5, 4)) {
		case ARITH:
			switch(OP) {
				case POW:
				{
					// Precisamos converter para double para usar a math.h
    				// x é a base, y é o expoente
    				// ATENÇÃO: Convertendo para 'short' antes para o C entender sinal negativo corretamente
    				base = (short)x; 
    				expoente = (short)y;

    				// Calcula a potência
    				resultado_real = pow(base, expoente);

    				// Converte de volta para inteiro
    				result = (int)resultado_real;

    				// --- Tratamento de Flags ---

    				// 1. Checa Overflow (se passou de +32767 ou menor que -32768)
    				if (resultado_real > 32767 || resultado_real < -32768) {
        				auxFRbits[ARITHMETIC_OVERFLOW] = 1;
    				} else {
        				auxFRbits[ARITHMETIC_OVERFLOW] = 0;
    				}

    				// 2. Checa Divisão por Zero (matematicamente 0 elevado a negativo é divisão)
    				if (base == 0 && expoente < 0) {
         				auxFRbits[DIV_BY_ZERO] = 1;
    				}

    				// 3. Checa Negativo (resultado final)
    				if (result & 0x8000) auxFRbits[NEGATIVE] = 1;
